...
1.  **Template Verification**: Before generating a new changelog block, inspect `.agent/resources/schemas/changelog_entry.template.md` and use its headings. Before reporting completion, run `npm run agent -- markdown:validate {changelog_entry_path} changelog_entry.template.md` when the entry is saved separately, or manually apply the same no-placeholder gate to the inserted changelog block.
