...
- Ensure the agent generates the detailed RCA report in `.workspaces/debug/{date}-rca-{slug}.md` (where `{date}` is today's date in `YYYY-MM-DD` format).
...
- **Suggested Command**: `/59-Wiki project ingest .workspaces/debug/{date}-rca-{slug}.md`
