...
4. **Design System & Output**: Run the search script to generate a design system. Save the final design system specification as a markdown report under `.workspaces/reports/{date}-ui-ux-{topic}.md` (where `{date}` is today's date in `YYYY-MM-DD` format and `{topic}` is a slugified version of the target screen or feature).
