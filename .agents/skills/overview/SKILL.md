---
name: overview
description: "[devflow][B] Validate and, when needed, normalize the two planning docs before generating devflow/context/project-overview.md from devflow/project-plan.md and devflow/build-plan.md. The overview is the single AI-facing source of truth that project instructions load every session. Use when the user runs /overview, invokes $overview, has just finished writing or editing the plans, asks to shape rough plans into the Blueprint format, or asks to regenerate the project overview."
---
...
You provide two files: `devflow/project-plan.md` (what & why) and
`devflow/build-plan.md` (the ordered feature list), drafted directly, through
...
step: it distills both plans into `devflow/context/project-overview.md`, the
...
- `devflow/project-plan.md` - problem, users, features, data, tech,
  monetization, UI/UX, deployment
- `devflow/build-plan.md` - the ordered, one-line-per-feature build checklist
...
Write `devflow/context/project-overview.md` (create `devflow/context/` if needed), following
