...
function discoverSharedBaseline(repoRoot: string, cloneRoot: string): string {
...
function main() {
  ...
  const baseline = tracking?.lastReviewedCommit || nexusComparisonBase;
  ...
  const nexusChanges = parseChanges(
    run(
      "git",
    ...
    JSON.stringify(
      {
        ...
        cloneRoot,
        trackingSource: tracking ? trackingRelativePath : "shared-git-history",
        nexusComparisonBase,
