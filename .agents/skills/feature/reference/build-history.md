...
Before freezing a new spec, check its exact planned archive path in `devflow/history/features/` and branch.
...
A build is reversed only by a completed `**Type:** Rollback` record in `devflow/history/rollbacks/` with its exact
