...
2. **Infrastructure node** — a host, VM, pod, or managed service inside a zone. The [SVG primitives](svg-primitives.md) node-box pattern at `rx=6`, with a rectangular type tag (`rx=2`, **not** a pill) carrying `POD` / `VM` / `MANAGED` / `CDN` in the top-left corner.
