...
- Nodes are the standard node-box pattern ([section 6](svg-primitives.md)): `rx=6`, 160px wide, 56px tall.
- **Fan-in badge.** Every node carries a Geist Mono 8px badge in its top-right corner, inside a small `rx=2` box, showing how many nodes depend on it (`4 in`). A mask fully inside a node is a badge chip, not a label — legitimate under [section 6](svg-primitives.md) rule 6. The node with the highest fan-in is the diagram's structural story; size nothing else to compete with it.
- **Node treatments** ([section 5](design.md) Node type → treatment):
