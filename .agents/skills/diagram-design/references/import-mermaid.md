...
- Reroute all connections with the [section 6](svg-primitives.md) connector rules. Mermaid edge length markers are ranking hints, not content.
- Do not add a component merely to fill space. Imports remain bounded by source meaning.

## Step 6 — Deliver

1. Write the self-contained HTML.
2. Run the [section 9](validation.md) taste gate and [`output-spec.md` §6](output-spec.md) checklist.
