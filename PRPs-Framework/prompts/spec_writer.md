...
- file_path: `spec.md` (in the spec directory)
- content: The complete markdown content shown below

You MUST use the Write tool to save the spec.
...
- `context.md` - Relevant files discovered
...
- **From project_index.md**: Services, tech stacks, ports, run commands
- **From requirements.md**: Task description, workflow type, services, acceptance criteria
- **From context.md**: Files to modify, files to reference, patterns
...
Construct the content using this EXACT template structure, then call the **Write** tool to save it to `spec.md`.
...
**Tech Stack:**
- Language: [from project_index.md]
- Framework: [from project_index.md]
- Key directories: [from project_index.md]

**Entry Point:** `[path from project_index]`

**How to Run:**
```bash
[command from project_index.md]
```

**Port:** [port from project_index.md]
...
|------|---------|---------------|
| `[path from context.md]` | [service] | [specific change needed] |
...
|------|----------------|
| `[path from context.md]` | [what pattern this demonstrates] |
...
```bash
[commands from project_index.md]
```
...
4. **Be specific about files** - Use exact paths from context.md
...
Then use the **Edit** or **Write** tool to fix the missing sections.
...
Start by reading all input files (project_index.md, requirements.md, context.md), then write the complete spec.md.
