...
cat "$SPEC_DIR/project_index.md" 2>/dev/null || echo "No project index"

# 7. Read the task context (files to modify, patterns to follow)
cat "$SPEC_DIR/context.md" 2>/dev/null || echo "No context file"
...
if [ -f "$SPEC_DIR/memory/codebase_map.md" ]; then
  echo "Codebase Map:"
  cat "$SPEC_DIR/memory/codebase_map.md"
else
...
if [ -d "$SPEC_DIR/memory/session_insights" ]; then
  ...
  ls -t "$SPEC_DIR/memory/session_insights/session_*.md" 2>/dev/null | head -3 | while read file; do
    echo "--- $file ---"
    cat "$file"
  done
else
...
# Read service commands from project_index.md
cat project_index.md | grep -A 5 '"dev_command"'
...
# Test connectivity (ports from project_index.md)
...
**Note**: Memory files (attempt_history.md, build_commits.md) are automatically
...
# Save insights
session_file = session_insights_dir / f"session_{session_num:03d}.md"
with open(session_file, "w") as f:
    f.write(f"# Session {session_num} Insights\n\n" + str(insights)) # Example placeholder

print(f"Session insights saved to: {session_file}")

# Update codebase map
if insights["discoveries"]["files_understood"]:
    map_file = memory_dir / "codebase_map.md"

    # Save
    with open(map_file, "a") as f:
        f.write(f"\n## Session {session_num} Additions\n" + str(insights["discoveries"]["files_understood"]))
