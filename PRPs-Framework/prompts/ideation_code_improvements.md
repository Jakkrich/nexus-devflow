...
- `memory/codebase_map.md` (if exists) - Previously discovered file purposes
- `memory/patterns.md` (if exists) - Established code patterns

**Output**: `code_improvements_ideas.md` with code improvement ideas
...
- **Rationale**: [Rationale]
- **Builds Upon**: [Feature]
- **Estimated Effort**: [trivial|small|medium|large|complex]
...
cat memory/codebase_map.md 2>/dev/null || echo "No codebase map yet"
cat memory/patterns.md 2>/dev/null || echo "No patterns documented"

# Look at existing roadmap if available (to avoid duplicates)
cat ../roadmap/roadmap.md 2>/dev/null | head -100 || echo "No roadmap"
...
If `graph_hints.md` exists and contains hints for `code_improvements`, use them to:
...
1. **Not Already Planned**: Check ideation_context.md for similar items
...
**You MUST create code_improvements_ideas.md with your ideas.**

```bash
cat > code_improvements_ideas.md << 'EOF'
# Code Improvement Ideas

## [ci-001]: [Title]
- **Description**: [What it does]
- **Rationale**: [Why the code reveals this]
- **Builds Upon**: [Feature]
- **Effort**: [trivial|small|medium|large|complex]
- **Affected Files**: [file1.ts]
...
Verify:
```bash
cat code_improvements_ideas.md
```
...
code_improvements_ideas.md created successfully.
...
Start by reading project_index.md and ideation_context.md, then search for patterns and opportunities across all effort levels.
