...
echo "Completed: $(grep -c "\[x\]" implementation_plan.md)"
echo "Pending: $(grep -c "\[ \]" implementation_plan.md)"
echo "In Progress: $(grep -c "\[/\]" implementation_plan.md)"
```
...
```bash
# Get test commands from project_index.md
cat project_index.md | grep "test_command"
...
### 6.0: Third-Party API/Library Validation (Manual)

**CRITICAL**: If the implementation uses third-party libraries or APIs, validate the usage against official documentation or established codebase patterns.
...
| Third-Party API Validation | ✓/✗ | [summary of findings] |
...
Update `implementation_plan.md` to record QA sign-off:

Add a `## QA Sign-off` section with status, timestamp, and summary of passed tests.
...
# Note: qa_report.md and implementation_plan.md are in the issue directory
...
# Note: QA_FIX_REQUEST.md and implementation_plan.md are in the issue directory
# Only commit actual code fixes to the project
```

Update `implementation_plan.md` with the rejected status and a link to `QA_FIX_REQUEST.md`.
