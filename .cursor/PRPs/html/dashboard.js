...
// ═══════════════════════════════════════════════
const IDB = { NAME: 'auto-claude-dash', STORE: 'kv', VER: 3 };
...
async function refreshData() {
  ...
  try {
    ...
    strategyDiscovery = await tryReadJson(rootHandle, '.auto-claude', 'roadmap_discovery.json');
    strategyRoadmap = await tryReadJson(rootHandle, '.auto-claude', 'roadmap.json');
...
async function loadFromHandle(handle) {
  let dirs = await listDirs(handle, '.auto-claude', 'specs');
...
function renderStrategy() {
  ...
  if (!strategyDiscovery && !strategyRoadmap) {
    wrap.innerHTML = '<div class="no-data"><i class="fa-solid fa-chess-knight"></i><h3>No strategy data</h3><p>Load a project with .auto-claude/roadmap_discovery.json and roadmap.json</p></div>';
    return;
  }
    ...
    html += `
          <div class="strategy-section">
            ...
            <div class="sec-grid">
              <div class="sec-card">
                <label>Vision</label>
                <div class="sec-val">${escHtml(d.product_vision?.one_liner || (d.vision ? d.vision : '—'))}</div>
              </div>
              <div class="sec-card">
                <label>Target Audience</label>
                <div class="sec-val">${escHtml(d.target_audience?.primary_persona || (d.target_audience?.primary ? d.target_audience.primary : '—'))}</div>
              </div>
              <div class="sec-card">
...
function renderKanban(specs = null) {
  ...
  if (!rootHandle && !allSpecs.length) {
    wrapper.innerHTML = '<div class="no-data"><i class="fa-solid fa-layer-group"></i><h3>No workspace loaded</h3><p>Click <strong>Browse</strong> to select a project folder containing <code>.auto-claude/specs</code>, or click <strong>Demo</strong> to preview sample data.</p></div>';
...
async function updateTaskStatus(specId, newStatus) {
  ...
  try {
    // Find spec directory
    let specsDir;
    try { specsDir = await rootHandle.getDirectoryHandle('.auto-claude'); specsDir = await specsDir.getDirectoryHandle('specs'); }
  ...
  const rowsHtml = rows.sort((a, b) => new Date(a.earliest) - new Date(b.earliest)).map(r => {
    const phaseBars = r.phases.length
      ? r.phases.filter(p => p.start).map(p => {
        ...
        return `<div class="tl-bar" style="left:${s}%;width:${w}%;background:${col};opacity:.8" title="${p.name}: ${formatDate(p.start)}${p.end ? ' → ' + formatDate(p.end) : '…'}">${w > 8 ? p.name : ''}</div>`;
      }).join('')
      : (() => {
...
function renderStats() {
  ...
  <div class="maturity-section">
    ...
    <div class="maturity-progress-wrap">
      <div class="maturity-progress-labels">
        ...
        <span class="maturity-progress-pct">${doneSubs}/${totalSubs} subtasks · <strong style="color:var(--accent2)">${progress}%</strong></span>
      </div>
  ...
  <div class="dash-grid">
    ...
    <div class="dash-card" style="grid-column:1/-1">
      <h4>Velocity — Done per week (last 8 weeks)</h4>
    ...
    for (const { field, text } of searchable) {
      ...
      if (idx >= 0) {
        ...
        if (!excerpt) {
          ...
          excerpt = (start > 0 ? '…' : '') + escHtml(text.slice(start, end)).replace(new RegExp(escHtml(q), 'gi'), m => `<mark>${m}</mark>`) + (end < text.length ? '…' : '');
          matchField = field;
        }
      }
    }
  ...
  container.innerHTML = results.map(r => {
    ...
    return `<div class="search-result-item" onclick="openModal(getSpecById('${r.spec.id}'))">
      ...
      <div class="sri-body">
        ...
        <div class="sri-path">${r.spec.id} · matched in ${r.matchField}</div>
...
function openModal(spec) {
  ...
  if (modalHeader) {
    ...
    if (status === 'done') {
      ...
      commandsToCopy = [
        { cmd: `/01-New ${spec.id}`, label: '01-New', title: `Copy /01-New (Initialize Spec)`, icon: 'fa-file-circle-plus' },
      ...
      commandsToCopy = [
        ...
        { cmd: `/10-Human Feedback ${spec.id} "แก้ไขส่วนนี้หน่อย..."`, label: 'Feedback', title: `Copy /10-Human Feedback`, icon: 'fa-comment-dots' }
      ];
  ...
  document.getElementById('tab-overview').innerHTML = `
    ...
    <div class="overview-grid cols-3">
      ...
      <div class="overview-item"><label>Detailed Status</label><value><span style="font-family:'DM Mono',monospace; font-size:11px; color:var(--text2)">${plan.planStatus || '—'}</span></value></div>
      ...
      <div class="overview-item" style="grid-column: span 3">
        ...
        <value style="display:block; line-height:1.6; color:var(--text2); font-size:12px; padding-top:4px">${spec.complexity?.reasoning || plan.complexity_assessment?.approach || '—'}</value>
      </div>
    </div>
    ...
    <div style="margin-top:20px; padding:12px; background:rgba(56,189,248,0.05); border:1px dashed var(--accent); border-radius:var(--r); display:flex; gap:12px; align-items:flex-start">
      ...
      <div style="font-size:11px; line-height:1.5; color:var(--text2)">
        ...
        ประเมินความซับซ้อน (Complexity) ร่วมกับ AI: หาก AI วิเคราะห์ว่า <strong>COMPLEX</strong> แต่ท่านมองว่า <strong>SIMPLE</strong> (หรือกลับกัน) 
        โปรดหยุดและ <strong>Discuss Spec</strong> เพื่อปรับจูนความเข้าใจและขอบเขตงานให้ตรงกันก่อนเริ่มลงมืดทำแผนงาน (Planning) ต่อไป
      </div>
    </div>
    ...
    <div class="phase-section">
      ...
      if (typeof v === 'object') {
        ...
        return `<div style="font-size:12px;color:var(--text3);margin-top:3px">
                    ...
                    ${exp ? `<div style="margin-top:2px;padding-left:14px;color:var(--text3)">→ ${escHtml(exp)}</div>` : ''}
                  </div>`;
      }
      return '';
    })() : ''}
  ...
  // ── Shared Vertical Stepper renderer ──
...
function copyText(text) {
  if (navigator.clipboard?.writeText) {
    navigator.clipboard.writeText(text)
      ...
      .catch(() => showToast('Clipboard unavailable — try HTTPS', 'error'));
  } else {
...
function loadDemoData() {
  ...
  allSpecs = [
    ...
    },
    {
      ...
      meta: { priority: 'medium', complexity: 'high', impact: 'high', category: 'feat' },
      specMd: '# PRPs Dashboard\n\n## Overview\nKanban board for monitoring auto-claude specs.\n\n## Requirements\n- Kanban view\n- Auto refresh\n- Detail modal\n- Timeline view\n- Stats dashboard',
      logs: {
        phases: {
...
async function tryRestoreLastSession() {
  ...
  if (permitted) {
    ...
    document.getElementById('projBarName').textContent = active.name + ' — click Browse';
  }
}
