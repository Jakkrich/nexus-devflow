...
try {
  ...
  assert(alpha.currentStage === 'spec', 'alpha current stage should be spec');
  assert(alpha.approvalStatus === 'Pending', 'alpha approval status should come from stage file');
  assert(alpha.nextAllowedCommand === '/implement 101', 'alpha next command should come from stage file');
  ...
  assert(beta.currentStage === 'check', 'beta current stage should be check');
  assert(beta.approvalStatus === 'Approved', 'beta approval status should come from stage file');
  assert(beta.nextAllowedCommand === '/complete 102', 'beta next command should come from stage file');
  ...
  assert(textResult.stdout.includes('/complete 102'), 'text summary should include next command for beta');
