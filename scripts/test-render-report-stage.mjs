...
`;
  ...
  writeFile(path.join(workspaceDir, 'report.md'), markdown);
...
`);
  ...
  assert(adapterResult.outputPath.endsWith('report.html'), 'adapter should target report.html');
...
`);
  ...
  writeFile(path.join(flatWorkspaceDir, 'report.md'), '# Shadow\n');
  ...
  const cliRun = spawnSync(
    process.execPath,
    [path.join(path.resolve('scripts'), 'render-html.mjs'), '--stage', 'report', workspaceDir],
    { cwd: path.resolve('.'), encoding: 'utf8' }
  );
  assert(cliRun.status === 0, `stage-aware render-html CLI should pass:\n${cliRun.stdout}\n${cliRun.stderr}`);
  assert(fs.existsSync(path.join(workspaceDir, 'report.html')), 'stage-aware render-html CLI should write report.html');

  const ambiguousRoot = path.join(scratchRoot, 'ambiguous-project');
  writeFile(path.join(ambiguousRoot, 'devflow', 'specs', '123-first', 'report.md'), '# First\n');
  writeFile(path.join(ambiguousRoot, 'devflow', 'specs', '123-second', 'report.md'), '# Second\n');
