...
- **Ticket-Driven TDD Bridge (Matt Pocock Flow)**: When implementing tracer-bullet tickets from `/to-tickets` via `/implement --ticket <NN>` or auto-frontier, each acceptance criterion executes via Red-Green-Refactor, ticking `- [x]` in the ticket file, marking status `done`, and mirroring progress into `devflow/context/{xxx-slug}/spec.md`.
