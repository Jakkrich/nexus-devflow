...
> Living context artifact generated by `scripts/overview.ts` on 2026-09-07T04:53:40.699Z.
...
- Verified/Dev commands:
  - build: `ยังไม่กำหนด`
  - test: `npm --prefix packages/create-nexus-devflow test && npm run test:overview && npm run test:sandbox && npm run test:run-state`
...
- **Feature**: ซิงก์ส่วนขยาย Upstream AI Blueprint (v1.5.3) — Manifest-Aware Adapter Selection in Onboard & E2E Scenario Parity (`REQ-20260904-001`)
- **Feature**: ซิงก์ส่วนขยาย Upstream AI Blueprint (v1.5.2) — Unborn Repository Onboarding & Branch Baseline Finalization (`REQ-20260903-001`)
- **Feature**: ซิงก์ส่วนขยาย Upstream AI Blueprint (v1.5.0 – v1.5.1) — Hardened Run-State Engine & On-Demand Context Loading (`DISC-20260903-001`)
- **Feature**: Role-Based Profiles & SA Analyze Command Engine (`DISC-20260903-001`)
- **Feature**: Prune Studio Reference Artifacts (`069-prune-studio-reference-artifacts`)
- **Feature**: Standalone HTML Report Diagram Integration & Skill Auto-Install (`DISC-20260902-003`)
- **Feature**: ซิงก์ส่วนขยาย Tooling & Evals Parity จาก Upstream AI Blueprint (E2E Agent Harness, Complete Evals Suite, Sandbox Automation, Upstream Drift Monitor) (`DISC-20260902-001`)
- **Feature**: ซิงก์ AI Blueprint Upstream v1.3.0 – v1.4.1 (Planning Baseline Commit, Overview 20KB Compactness & Efficient Workflow) (`DISC-20260902-002`)
...
- โฟลเดอร์หลัก: assets, benchmarks, devflow, docs, evals, packages, prototypes, scripts
