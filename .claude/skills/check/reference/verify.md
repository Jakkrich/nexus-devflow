...
Read `devflow/config.json` first. A missing file means the built-in defaults
...
Read `devflow/context/{xxx-slug}/spec.md`. Pull the observable "done when"
...
If there's no active feature spec, ask what to verify rather than guessing.
...
- All proven -> update only the `**Status:**` line in
  `devflow/context/{xxx-slug}/spec.md` to `verified`, then say it is ready for
  `/complete`.
...
- **Check the spec, not vibes.** Verify against the done-whens in
  `spec.md`, so "works" means what the spec said it would do.
