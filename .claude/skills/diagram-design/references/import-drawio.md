...
Fresh layout on the 4px grid, per the type reference and [section 6](svg-primitives.md)–[section 7](layout.md). Explicitly:
...
| no fill | unstyled | Backend/API |
  ...
  Source color is a *signal about role*, not a color to keep. Six fill colors in the source do not become six fills in the output — the palette is one accent plus the ink ramp ([section 5](design.md)).
...
- **Reroute every connector.** Source waypoints are dead weight — the digest reports a waypoint count so you know how tangled the original was, not so you can reproduce it. Rounded orthogonal elbows, fanned attach points, no overlaps: [section 6](svg-primitives.md) rules 1–5, no exceptions for imported content.
- **Set the `viewBox` from the size preset**, then lay out inside it — don't draw first and crop after.

## Step 6 — Deliver

1. Write the `.html`.
2. Run the [section 9](validation.md) taste gate **and** the [`output-spec.md` §6](output-spec.md) checklist.
...
12 source nodes → 8 drawn, inside the standard [layout budget](layout.md) even at a level that allows 12.
...
| Preserving draw.io diagonal connectors | Orthogonal elbows are mandatory ([section 6](svg-primitives.md) rule 1) regardless of origin |
