...
- **15 Slash Commands** (\`${compoundRefPath}/commands/<command>.md\`): \`hunt\`, \`recon\`, \`triage\`, \`validate\`, \`chain\`, \`report\`, \`scope\`, \`token-scan\`, \`surface\`, \`autopilot\`, ฯลฯ
...
1. **During \`/check\` (Security & QA Verification)**:
   - When verifying a feature dealing with Auth, Multi-tenancy, or Data Exports, run a BugHunter self-assessment:
     \`"เปิดอ่าน ${compoundRefPath}/skills/hunt-idor/SKILL.md เพื่อทดสอบหาช่องโหว่ IDOR และ Token Tampering ใน Endpoint นี้"\`
