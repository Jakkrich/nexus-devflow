...
test("readDiscoveries returns recent items and handles a missing root", async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "devflow-discoveries-"));
  try {
    assert.equal((await readDiscoveries(tempDir)).total, 0);
    const dir = path.join(tempDir, "devflow", "discoveries", "DISC-20260822-001-one");
    await fs.mkdir(dir, { recursive: true });
    await fs.writeFile(path.join(dir, "00-explore.md"), "# Discovery Document: [DISC-20260822-001] One\n> **Status**: `Defer`");
